...
public class BCClient {
	private static String XMLfileName = "C:\\temp\\mimer2.output";
	...
	public static void main(String args[]) {
		...
		try {
			...
			do {
				System.out
						.print("Enter a string to send to back channel of webserver, (quit) to end: ");
				System.out.flush();
				...
				if (userData.indexOf("quit") < 0) {
					...
					daTest = (myDataArray) xstream.fromXML(xml); // deserialize
																	// data
					...
					if (xmlFile.createNewFile() == false) {
						...
						toXmlOutputFile.println("First arg to Handler is: "
								+ argOne + "\n");
		...
		try {
			...
			if (xmlFile.createNewFile() == false) {
				...
				toXmlOutputFile.println("First arg to Handler is: " + argOne
						+ "\n");
				toXmlOutputFile
						.println("<This where the persistent XML will go>");
				toXmlOutputFile.close();
			}
		} catch (Throwable e) {
			e.printStackTrace();
